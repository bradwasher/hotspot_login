...
def main():
    ...
    print("""
[+] *****************************************************************************************
[+] Mikrotik Hotspot Login Script v1.0
[+] *****************************************************************************************
[+]""")
    args = get_args()
...
def login(target, username, password, check_logged_in=True):
    ...
    try:
        ...
        if check_logged_in:
            if re.search('Welcome', login_page.text) or re.search('logged in', login_page.text):
                print(f'[+] Device is already logged in at {target}')
                print()
                exit(0)

        # get url of login page
        # no chap -> <form name="login" action="http://192.168.88.10/login" method="post"
        # chap -> <form name="sendin" action="http://192.168.88.10/login" method="post"
        login_pattern = re.search('form name=".*in" action="(.*)" method="post"', login_page.text)
        ...
        if salt_pattern is None:
            ...
            # hash password with salt values
        ...
        # post data to target of the "sendin form
        ...
        if not re.search('Welcome', login_response.text) and not re.search('logged in', login_response.text):
            print(f'[!] Fail - Unsuccessful login at {login_url} with credentials "{username}" and "{password}"')
            return False
    ...
    print(f'[+] Success - Logged in at {login_url} with credentials "{username}" and "{password}"')
    return True
...
if __name__ == "__main__":
    ...
    """
    Example of form with salted and hashed password
